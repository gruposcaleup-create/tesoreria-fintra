...
import { exportCapTable, ExcelColumn } from "@/lib/export-reportes"
...
/** Filter and map EgresoContable records for Chapter 1000 */
function buildCap1000Rows(
    ...
    return egresos
        .filter((e) => {
            ...
            return stripped.length > 0 && stripped[0] === "1"
        })
        .map((e) => ({
...
export function Cap1000Table() {
    const { egresosContables, presupuesto, fiscalConfig } = useTreasury()

    const rows = useMemo(
        () => buildCap1000Rows(egresosContables, presupuesto),
    ...
    const handleExport = () => {
        exportCapTable({
            fileName: "ANEXO_CAP_1000_SERVICIOS_PERSONALES",
            title: "ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 1000",
            subTitle: "CAPÍTULO 1000 SERVICIOS PERSONALES",
            fiscalConfig,
            rows,
            totalRow: { label: "TOTAL SERVICIOS PERSONALES", value: total },
            columns: [
        ...
        <div className="w-full overflow-x-auto">
            ...
            <div className="mb-4 space-y-2 print:mb-2">
                ...
                <h2 className="text-sm font-black tracking-wide uppercase pt-2">
                    ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 1000
                </h2>
            </div>

            {/* ═══ TABLE ═══ */}
            <table className="w-full border-collapse text-[9px] min-w-[1100px]">
                ...
                <tbody>
                    ...
                    <tr className="bg-[#f2f2f2]">
                        <td colSpan={8} className={`${tdBase} text-center font-bold text-[10px] bg-[#f2f2f2]`}>
                    ...
                    {rows.map((row, idx) => (
                        <tr key={idx} className="hover:bg-muted/30">
                            ...
                            <td className={tdLeft} title={row.descripcionPartida}>
                                <span className="max-w-[220px] block truncate">{row.descripcionPartida}</span>
                            </td>
                            <td className={tdLeft} title={row.concepto}>
                                <span className="max-w-[240px] block truncate">{row.concepto}</span>
                            </td>
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={8} className="text-center text-muted-foreground text-[9px] py-4 border border-black/10">
                                Sin registros para el Capítulo 1000
                            </td>
                        </tr>
                    )}
                    ...
                    <tr className="bg-[#dce6f1] font-bold">
                        <td colSpan={7} className={`${tdBase} text-right font-bold text-[10px]`}>
