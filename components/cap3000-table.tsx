...
function resolveCapitulo(cog: string, tree: PresupuestoItem[]): string {
    ...
    return `${firstDigit}000 SERVICIOS GENERALES`
}
...
function buildCap3000Rows(
    ...
    return egresos
        .filter((e) => {
            ...
            return stripped.length > 0 && stripped[0] === "3"
        })
        .map((e) => ({
...
export function Cap3000Table() {
    const { egresosContables, presupuesto, fiscalConfig } = useTreasury()

    const rows = useMemo(
        () => buildCap3000Rows(egresosContables, presupuesto),
    ...
    const handleExport = () => {
        exportCapTable({
            fileName: "ANEXO_CAP_3000_SERVICIOS_GENERALES",
            title: "ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 3000",
            subTitle: "CAPÍTULO 3000 SERVICIOS GENERALES",
            fiscalConfig,
            rows,
            totalRow: { label: "TOTAL SERVICIOS GENERALES", value: total },
            columns: [
        ...
        <div className="w-full overflow-x-auto">
            ...
            <div className="mb-4 space-y-2 print:mb-2">
                ...
                <h2 className="text-sm font-black tracking-wide uppercase pt-2">
                    ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 3000
                </h2>
            </div>
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={10} className="text-center text-muted-foreground text-[9px] py-4 border border-black/10">
                                Sin registros para el Capítulo 3000
                            </td>
                        </tr>
                    )}
