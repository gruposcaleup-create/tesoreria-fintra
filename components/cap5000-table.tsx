...
// ── Column headers exactly matching CSV (10 columns) ──
const COLUMN_HEADERS = [
    "CAPÍTULO DE GASTO",
    "PARTIDA ESPECÍFICA",
    "DESCRIPCIÓN PARTIDA ESPECÍFICA",
    "CONCEPTO",
    "PROVEEDOR",
    "RFC DE PROVEEDOR",
    "NÚM. DE PÓLIZA",
...
function resolveDescripcion(cog: string, tree: PresupuestoItem[]): string {
    for (const item of tree) {
        if (item.codigo === cog || item.cog === cog) return item.descripcion
        if (item.subcuentas) {
            const found = resolveDescripcion(cog, item.subcuentas)
...
function resolveCapitulo(cog: string, tree: PresupuestoItem[]): string {
    ...
    return `${firstDigit}000 BIENES MUEBLES, INMUEBLES E INTANGIBLES`
}
...
// ── Mapped row type (10 columns) ──
interface Cap5000Row {
...
function buildCap5000Rows(
    ...
    return egresos
        .filter((e) => {
            ...
            return stripped.length > 0 && stripped[0] === "5"
        })
        .map((e) => ({
            capituloDeGasto: resolveCapitulo(e.cog, presupuesto),
            partidaEspecifica: e.cog,
            descripcionPartida: resolveDescripcion(e.cog, presupuesto) || e.cog,
            concepto: e.concepto,
            proveedor: e.pagueseA || "",
            rfcProveedor: "",
...
export function Cap5000Table() {
    const { egresosContables, presupuesto, fiscalConfig } = useTreasury()

    const rows = useMemo(
        () => buildCap5000Rows(egresosContables, presupuesto),
    ...
    const handleExport = () => {
        exportCapTable({
            fileName: "ANEXO_CAP_5000_BIENES_MUEBLES",
            title: "ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 5000",
            subTitle: "CAPÍTULO 5000 BIENES MUEBLES, INMUEBLES E INTANGIBLES",
            fiscalConfig,
            rows,
            totalRow: { label: "TOTAL BIENES MUEBLES, INMUEBLES E INTANGIBLES", value: total },
            columns: [
                { header: "CAPÍTULO DE GASTO", key: "capituloDeGasto", width: 25 },
                { header: "PARTIDA ESPECÍFICA", key: "partidaEspecifica", width: 15 },
                { header: "DESCRIPCIÓN PARTIDA ESPECÍFICA", key: "descripcionPartida", width: 30 },
                { header: "CONCEPTO", key: "concepto", width: 35 },
                { header: "PROVEEDOR", key: "proveedor", width: 25 },
                { header: "RFC DE PROVEEDOR", key: "rfcProveedor", width: 15 },
        ...
        <div className="w-full overflow-x-auto">
            ...
            <div className="mb-4 space-y-2 print:mb-2">
                ...
                <h2 className="text-sm font-black tracking-wide uppercase pt-2">
                    ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 5000
                </h2>
            </div>

            {/* ═══ TABLE ═══ */}
            <table className="w-full border-collapse text-[9px] min-w-[1300px]">
                ...
                <tbody>
                    ...
                    <tr className="bg-[#f2f2f2]">
                        <td colSpan={10} className={`${tdBase} text-center font-bold text-[10px] bg-[#f2f2f2]`}>
                    ...
                    {rows.map((row, idx) => (
                        <tr key={idx} className="hover:bg-muted/30">
                            <td className={tdLeft} title={row.capituloDeGasto}>
                                <span className="max-w-[180px] block truncate">{row.capituloDeGasto}</span>
                            </td>
                            <td className={tdCenter}>{row.partidaEspecifica}</td>
                            <td className={tdLeft} title={row.descripcionPartida}>
                                <span className="max-w-[200px] block truncate">{row.descripcionPartida}</span>
                            </td>
                            <td className={tdLeft} title={row.concepto}>
                                <span className="max-w-[200px] block truncate">{row.concepto}</span>
                            </td>
                            <td className={tdLeft} title={row.proveedor}>
                                <span className="max-w-[160px] block truncate">{row.proveedor}</span>
                            </td>
                            <td className={tdCenter}>{row.rfcProveedor}</td>
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={10} className="text-center text-muted-foreground text-[9px] py-4 border border-black/10">
                                Sin registros para el Capítulo 5000
                            </td>
                        </tr>
                    )}
                    ...
                    <tr className="bg-[#dce6f1] font-bold">
                        <td colSpan={9} className={`${tdBase} text-right font-bold text-[10px]`}>
