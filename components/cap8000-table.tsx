...
function resolveCapitulo(cog: string, tree: PresupuestoItem[]): string {
    ...
    return `${firstDigit}000 PARTICIPACIONES Y APORTACIONES`
}
...
function buildCap8000Rows(
    ...
    return egresos
        .filter((e) => {
            ...
            return stripped.length > 0 && stripped[0] === "8"
        })
        .map((e) => ({
...
export function Cap8000Table() {
    const { egresosContables, presupuesto, fiscalConfig } = useTreasury()

    const rows = useMemo(
        () => buildCap8000Rows(egresosContables, presupuesto),
    ...
    const handleExport = () => {
        exportCapTable({
            fileName: "ANEXO_CAP_8000_PARTICIPACIONES",
            title: "ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 8000",
            subTitle: "CAPÍTULO 8000 PARTICIPACIONES Y APORTACIONES",
            fiscalConfig,
            rows,
            totalRow: { label: "TOTAL PARTICIPACIONES Y APORTACIONES", value: total },
            columns: [
        ...
        <div className="w-full overflow-x-auto">
            ...
            <div className="mb-4 space-y-2 print:mb-2">
                ...
                <h2 className="text-sm font-black tracking-wide uppercase pt-2">
                    ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 8000
                </h2>
            </div>
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={7} className="text-center text-muted-foreground text-[9px] py-4 border border-black/10">
                                Sin registros para el Capítulo 8000
                            </td>
                        </tr>
                    )}
