...
function resolveCapitulo(cog: string, tree: PresupuestoItem[]): string {
    ...
    return `${firstDigit}000 TRANSFERENCIAS, ASIGNACIONES, SUBSIDIOS Y OTRAS AYUDAS`
}
...
// ── Mapped row type (8 columns) ──
interface Cap4000Row {
...
function buildCap4000Rows(
    ...
    return egresos
        .filter((e) => {
            ...
            return stripped.length > 0 && stripped[0] === "4"
        })
        .map((e) => ({
...
export function Cap4000Table() {
    const { egresosContables, presupuesto, fiscalConfig } = useTreasury()

    const rows = useMemo(
        () => buildCap4000Rows(egresosContables, presupuesto),
    ...
    const handleExport = () => {
        exportCapTable({
            fileName: "ANEXO_CAP_4000_TRANSFERENCIAS",
            title: "ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 4000",
            subTitle: "CAPÍTULO 4000 TRANSFERENCIAS, ASIGNACIONES, SUBSIDIOS Y OTRAS AYUDAS",
            fiscalConfig,
            rows,
            totalRow: { label: "TOTAL TRANSFERENCIAS, ASIGNACIONES, SUBSIDIOS Y OTRAS AYUDAS", value: total },
            columns: [
        ...
        <div className="w-full overflow-x-auto">
            ...
            <div className="mb-4 space-y-2 print:mb-2">
                ...
                <h2 className="text-sm font-black tracking-wide uppercase pt-2">
                    ANEXO DE REGISTROS CONTABLES DEL CAPÍTULO 4000
                </h2>
            </div>
                    ...
                    {rows.length === 0 && (
                        <tr>
                            <td colSpan={8} className="text-center text-muted-foreground text-[9px] py-4 border border-black/10">
                                Sin registros para el Capítulo 4000
                            </td>
                        </tr>
                    )}
